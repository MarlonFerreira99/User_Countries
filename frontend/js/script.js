...
async function start() {
  // "Normal"
  // await fetchUsers(); //estao usando o "await" pois usam promises na sua funcao
  // await fetchCountries(); //estao usando o "await" pois usam promises na sua funcao
  ...
  // "Promise paralela = funciona simultaneamente"
...
function mergeUsersAndCountries() {
  ...
  globalUsers.forEach( user => {
    const country = globalCountries.find( country => 
      country.countryId === user.userCountry
    );
...
function render() {
  ...
  divUsers.innerHTML = `
    <div class='row'>
      ${globalFilteredUserCountries.map(
        ({countryFlag, countryName, userPicture, userName }) => {
        return`
